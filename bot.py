...
async def handle_approval(update: Update, context: CallbackContext):
    ...
    if pi:
        ...
        save_data(bot_data)
        pdf = generate_pdf(pi)
        await context.bot.send_document(pi['user_id'], document=pdf, company_safe = "".join(x for x in pi.get("customer", "Quote") if x.isalnum() or x in " ").replace(" ", "_"); filename=f"{q_num}{company_safe}.pdf", caption="Approved!")
...
def main():
    app = Application.builder().token(os.environ.get("PI_BOT_TOKEN")).build()
